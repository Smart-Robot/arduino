...
//w_printf("_i2caddr[%x] control[%x] c[%x]\n", _i2caddr, control, c);
   ...
   IIC_Start();
   Write_IIC_Byte(_i2caddr<<1/*0x78*/);  //Slave address,SA0=0
   Write_IIC_Byte(0x00);	//write command
   Write_IIC_Byte(c/*IIC_Command*/);
   IIC_Stop();
  }
...
void Write_IIC_Byte(uint8_t IIC_Byte)
{
  ...
  for(i=0;i<8;i++)
  {
    if((IIC_Byte << i) & 0x80)digitalWrite(SDA, HIGH);
    else
      digitalWrite(SDA, LOW);
    digitalWrite(SCL, HIGH);
    digitalWrite(SCL, LOW);
